...
import { RegrasModel } from '../models/regrasModel';
import * as moment from 'moment';

export class RegrasController {

    private regrasModel = new RegrasModel;
     ...
     */
    validaRegra(regras: Array<RegraHorario>){
        return new Promise((response, erro)=>{
            ...
            this.iterar( 0, regras, (pos, regras, next)=>{
                ...
                if( regras[pos].day ) regras[pos].day = String(moment(regras[pos]["day"], "DD-MM-YYYY").unix());
    ...
    listarRegraFiltrada( req, res ){
        ...
        this.regrasModel.listarRegraFiltrada( String(dia_inicial), String(dia_final) ).then((regras: Array<any>)=>{
    ...
    listarRegras(req, res){
        let regras_final = [];
        this.regrasModel.listarTodasRegras().then((data: any)=>{
            data.map(regra=>{
     ...
     */
    iterar(pos, array: any, iterator, done) {
		if (pos < array.length) {
